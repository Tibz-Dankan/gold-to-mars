...
export class TravellingScene extends Scene {
  ...
  create() {
    ...
    // Add rocket
    this.rocket = this.add.image(this.scale.width / 2, this.scale.height - 100, "rocket");
    this.rocket.setOrigin(0.5);
    this.rocket.setAngle(45); // Tilt 45 degrees in Earth's atmosphere
  ...
  updatePhase() {
    ...
    if (this.elapsedTime === 30) {
      this.phase = "space";
      this.renderSpace();
      this.rocket.setAngle(90); // Rotate 90 degrees in space
    } else if (this.elapsedTime === 55) {
      this.phase = "mars";
      this.renderMarsAtmosphere();
      this.rocket.setAngle(135); // Rotate 135 degrees in Mars's atmosphere
    }
  }
